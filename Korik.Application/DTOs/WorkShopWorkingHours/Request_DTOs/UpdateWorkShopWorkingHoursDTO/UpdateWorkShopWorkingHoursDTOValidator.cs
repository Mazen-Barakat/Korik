...
{
    public class UpdateWorkShopWorkingHoursDTOValidator : AbstractValidator<UpdateWorkShopWorkingHoursDTO>
    {
        private readonly IWorkShopWorkingHoursService _workingHoursService;

        public UpdateWorkShopWorkingHoursDTOValidator(IWorkShopWorkingHoursService workingHoursService)
        {
            _workingHoursService = workingHoursService;

            RuleFor(x => x.Id)
                .GreaterThan(0).WithMessage("Working hours ID is required.");
            ...
            RuleFor(x => x)
                ...
                {
                    var existsResult = await _workingHoursService.WorkingHourExistsForDayAsync(model.WorkShopProfileId, model.Day, model.Id);
